...
public class AdditivePhylogeny {
	...
	static class Node{
		...
		public Node(String label) {
			if (label == null){
				this.label = ""+generator.getAndIncrement();
			}else{
				this.label=label;
			}
		}

		public Node addChild(String label,int distance){
		...
		public Node setParent(String label,int distance){
			Node parent = new Node(label);
			if (this.parent != null){
				throw new IllegalStateException("Parent of "+this.label+" is already set to "+this.parent.label+" and cannot be set to "+label);
			}else{
				this.parent = parent;
		...
		public SortedSet<String> edges(){
			SortedSet<String> edges = new TreeSet<String>((s1,s2)->{
				int nodeId1 = Integer.parseInt(s1.split("->")[0]);
				int nodeId2 = Integer.parseInt(s2.split("->")[0]);
				if (nodeId1 == nodeId2){
					int dest1 = Integer.parseInt(s1.split("->")[1].split(":")[0]);
					int dest2 = Integer.parseInt(s2.split("->")[1].split(":")[0]);
					return dest1-dest2;
				}else{
			...
			if (parent!=null){
				edges.add(label+"->"+parent.label+":"+distance);
			}
			for (Node n:children){
				edges.add(label+"->"+n.label+":"+n.distance);
			}
	...
	private static int[] findaPair(int n,int[][] DM){
		for (int i=0;i<n-1;i++){
			for (int k = 0;k<n-1;k++){
				if (i !=k && DM[i][k] == DM[i][n-1]+DM[n-1][k]){
					return new int[]{i,k};
				}
			}
		}
	...
	public static Node additivePhylogeny(int n,int[] D,Node nod,List<String> labels){
		if (generator == null){
			generator = new AtomicInteger(n);
		}
		...
		if (n == 2){
			...
			nod.addChild(labels.get(0),distance);
			return nod;
			
		}else{
			int l = n-1;
			
			int[] limb = EvolutionaryTree.limb(n, D, l);
			int limbLength = limb[0];
			
			
			
			System.out.println(limbLength);
			if (nod == null){
				nod = new Node(labels.get(n-1));
			}
			Node parent = nod.setParent(null, limbLength);
			
			int DM[][] = EvolutionaryTree.DM(D);
			
			IntStream.range(0, n-1).parallel().forEach(j->{
				DM[j][n-1] = DM[j][n-1]-limbLength; 
				DM[n-1][j] = DM[j][n-1];  
			});
			
			int[] pair = findaPair(n,DM);
			int i = pair[0],k=pair[1];
			
			int x = DM[k][l];
			parent.addChild(labels.get(k), x);
			...
			int [] dmr = IntStream.range(0, n).boxed().filter(ii->ii!=k).flatMap(ii->{
				return IntStream.range(0, n).filter(kk->kk!=k).map(kk->DM[ii][kk]).boxed();
			}).mapToInt(Integer::intValue).toArray();
			
			labels.set(l, parent.label);
			labels.remove(k);
			
			
			return additivePhylogeny(n-1, dmr,parent,labels);
		}
	...
	public static void main(String[] args) throws IOException{
		List<String> lines = IOUtils.readLines(EvolutionaryTree.class.getResourceAsStream("dataset_10330_6.txt"));
		/*
		...
		*/
		...
		int[] distanceMatrix = EvolutionaryTree.fromLines(lines.subList(1, lines.size()));
		List<String> labels = IntStream.range(0, n).boxed().map(i->""+i).collect(Collectors.toList());
		
		Node node = additivePhylogeny(n, distanceMatrix,null,labels);
		
		FileWriter out = new FileWriter("out.txt");
		
		IOUtils.writeLines(node.edges(), null, out);
		out.flush();
		out.close();
	}
